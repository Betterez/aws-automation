...
class ServiceInstaller
    ...
    def initialize(service_setup_data, aws_settings_data)
        ...
        when 'systemd'
            ...
            if service_setup_data['machine'].has_key? ('runner_command')
              @service_code.gsub!('[runner_command]',service_setup_data['machine']['runner_command'])
            elsif  service_setup_data['machine'].has_key? ('start')
              @service_code.gsub!('[runner_command]',run_command)
            end
            if service_setup_data['machine'].has_key?('runner_path')
            ...
            if service_setup_data.has_key?(:build_number)
