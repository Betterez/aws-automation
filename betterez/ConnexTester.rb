...
class ConnexTester
  ...
  def load_connex_servers
    filters=[
      { name: 'tag:Repository', values: ["connex2"] },
  ...
  def send_alarm(server)
    ...
    sns_client.publish({
      ...
      message: "connex2 server is down @ #{server}",
      subject: "betterez",
      })
  end
      ...
      @servers.each do |server|
        if check_server_availability server
          ...
          while (loops < 3) do
            ...
            puts server.run_ssh_command "sudo service connex restart"
            puts server.run_ssh_command "sudo service connex2 restart"
            loops+=1
            sleep 20
            if check_server_availability server
