...
class ServerCreator
  ...
  def create_server_name(service_setup_data)
    t = Time.new
    "#{service_setup_data[:repo]}_#{format('%d_%.2d_%.2d', t.year, t.month, t.day)}_#{Helpers.create_random_string(5)}"
  end
  ...
  def self.arrange_only_last_servers(servers)
    ...
    servers.each do |server|
      if servers_hash.key?(server[:serviceName])
  ...
  def deploy_to_elb(service_setup_data, servers)
    ...
    if service_setup_data['deployment']['elb_version'] == 1
      ...
      throw 'no elb found' if elbs.nil? || elbs.empty?
